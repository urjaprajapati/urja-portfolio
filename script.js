...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Category accent used for the abstract preview thumbnail (site screenshots
  // aren't embedded here since these are third-party sites) — thumbnail is a
  // stylized browser mockup, not a real capture of the destination page.
  const CAT_GRADIENT = {
    ...
    grid.innerHTML = list.map(p => `
      <article class="project-card" data-cat="${p.cat}">
        <a href="${p.url}" target="_blank" rel="noopener" class="project-thumb" style="background:${CAT_GRADIENT[p.cat]}">
          ...
          <span class="project-thumb-mark">${initials(p.name)}</span>
        </a>
